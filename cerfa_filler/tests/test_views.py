...
from ..models import BeneficiaryOrganization, Companies, DeclarativeStructure
...
class CompaniesViewTest(TestCase):
    ...
    def test_companies_create_view(self):
        response = self.client.post(
            reverse("cerfa_filler:companies-create"),
            {
                "label": "New Company",
                "repository_code": "12345",
                "postal_code": "75003",
    ...
    def test_companies_update_view(self):
        response = self.client.post(
            reverse("cerfa_filler:companies-update", args=[self.company.uuid]),
            {
    ...
    def test_companies_cerfa_pdf_view(self):
        response = self.client.get(
            reverse(
                "cerfa_filler:companies-cerfa-pdf", args=[self.company.uuid]
            )
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response["Content-Type"], "application/pdf")
        self.assertIn(
            'attachment; filename="recu_fiscal_don-',
            response["Content-Disposition"],
        )

    def test_update_valid_date_view(self):
        response = self.client.post(
            reverse("cerfa_filler:update-valid-date"),
            {"selected_companies": [self.company.uuid]},
        )
        self.company.refresh_from_db()
        self.assertEqual(response.status_code, 302)  # Check for redirect
        self.assertIsNotNone(
            self.company.valid_date
        )  # Check if valid_date is updated
